...
nsplug meta_vehicle.moos targ_$VNAME.moos $NSFLAGS WARP=$TIME_WARP \
       ...
       MMOD=$MMOD                   VPORT=$MOOS_PORT     \
       FSEAT_IP=$FSEAT_IP

# Extract numeric x,y from START_POS for BHV_Waypoint (needs "val,val" not "x=val,y=val")
WAYPT_POS=$(echo $START_POS | sed 's/x=\([^,]*\),y=\([^,]*\).*/\1,\2/')

nsplug meta_vehicle.bhv targ_$VNAME.bhv $NSFLAGS         \
       START_POS=$WAYPT_POS         VNAME=$VNAME         \
       STOCK_SPD=$STOCK_SPD         MMOD=$MMOD           \
       GOTO_LOC=$GOTO_LOC           LOITER_POS=$GOTO_LOC
